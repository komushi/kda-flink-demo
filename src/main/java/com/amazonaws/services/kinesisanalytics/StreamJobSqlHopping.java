...
import com.amazonaws.services.kinesisanalytics.sink.SinkDataApiHopping;
...
public class StreamJobSqlHopping {

    private static final Logger log = LoggerFactory.getLogger(StreamJobSqlHopping.class);
    ...
    private static SinkDataApiHopping createSinkFromStaticConfig() throws Exception {
        Map<String, Properties> applicationProperties = KinesisAnalyticsRuntime.getApplicationProperties();

        Properties sinkConfigProperties = applicationProperties.get("SinkConfigProperties");

        SinkDataApiHopping sink = new SinkDataApiHopping(sinkConfigProperties);
        return sink;
    }
    ...
    public static void main(String[] args) throws Exception {
        try {
            log.warn("StreamJobSqlHopping main");
...
//                        log.warn(object.toString());
            ...
            Table resultTable = tableEnv.sqlQuery(""+
                ...
                "  HOP_START(rowtime, INTERVAL '0' SECOND, " + "INTERVAL '" + interval + "' " + uom + ") as WINDOW_START, " +
                "  HOP_END(rowtime, INTERVAL '0' SECOND, " + "INTERVAL '" + interval + "' " + uom + ") as WINDOW_END, " +
                "  HOP_ROWTIME(rowtime, INTERVAL '0' SECOND, " + "INTERVAL '" + interval + "' " + uom + ") as RECEIVED_ON " +
                " FROM Inputs " +
                " GROUP BY HOP(rowtime, INTERVAL '0' SECOND, " + "INTERVAL '" + interval + "' " + uom + "), CAST (N02_001 AS VARCHAR(10))"
            );

            //Convert the Dynamic Table to a DataStream
            TupleTypeInfo<Tuple5<String, Long, Timestamp, Timestamp, Timestamp>> tupleType = new TupleTypeInfo<>(
                Types.STRING,
                Types.LONG,
                Types.SQL_TIMESTAMP,
                Types.SQL_TIMESTAMP,
            ...
            DataStream<Tuple5<String, Long, Timestamp, Timestamp, Timestamp>> resultSet = tableEnv.toAppendStream(resultTable, tupleType);

            resultSet
                .map((Tuple5<String, Long, Timestamp, Timestamp, Timestamp> value) -> {
                    String output = "CLASS: " + value.f0 + " CNT: " + value.f1 + " from: " + value.f2 + " to: " + value.f3 + "\n";
                    log.warn("resultSet output: " + output);

                    return value;
                })
                .returns(Types.TUPLE(Types.STRING, Types.LONG, Types.SQL_TIMESTAMP, Types.SQL_TIMESTAMP, Types.SQL_TIMESTAMP))
