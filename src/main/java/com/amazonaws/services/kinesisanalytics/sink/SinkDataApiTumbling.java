...
public class SinkDataApiTumbling extends RichSinkFunction<Tuple4<String, Long, Timestamp, Timestamp>>
...
//    private transient TupleState tupleState;
    ...
    private static final Logger log = LoggerFactory.getLogger(SinkDataApiTumbling.class);

    public SinkDataApiTumbling(Properties configProps) {
...
//            log.warn("snapshotState!");
            ...
            synchronized(tupleState.pendingTuplesPerCheckpoint) {
                List<Tuple4<String, Long, Timestamp, Timestamp>> checkpointedTupleList = tupleState.pendingTuplesPerCheckpoint.get(checkpointId);
...
//            log.warn("notifyCheckpointComplete!");
            ...
            synchronized(tupleState.pendingTuplesPerCheckpoint) {
                Iterator<Map.Entry<Long, List<Tuple4<String, Long, Timestamp, Timestamp>>>> pendingCheckpointsIt =
                        tupleState.pendingTuplesPerCheckpoint.entrySet().iterator();

                while (pendingCheckpointsIt.hasNext()) {
                    Map.Entry<Long, List<Tuple4<String, Long, Timestamp, Timestamp>>> entry = pendingCheckpointsIt.next();
                    Long pastCheckpointId = entry.getKey();
                    List<Tuple4<String, Long, Timestamp, Timestamp>> checkpointedTuples = entry.getValue();
    ...
    static final class TupleState implements Serializable {
         ...
         */
        public List<Tuple4<String, Long, Timestamp, Timestamp>> pendingTuples = new ArrayList<>();
         ...
         */
        public final Map<Long, List<Tuple4<String, Long, Timestamp, Timestamp>>> pendingTuplesPerCheckpoint = new HashMap<>();
