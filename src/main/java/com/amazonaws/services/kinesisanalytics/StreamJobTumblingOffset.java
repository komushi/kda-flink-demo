...
public class StreamJobTumblingOffset {

    private static final Logger log = LoggerFactory.getLogger(StreamJobTumblingOffset.class);
    ...
    public static void main(String[] args) throws Exception {
        try {
            log.warn("StreamJobTumblingOffset main");
            ...
            String interval = processorConfigProperties.getProperty("INTERVAL_IN_SECOND");
            String offset = processorConfigProperties.getProperty("OFFSET_IN_SECOND");
...
//                        log.warn(object.toString());
            ...
            inputStreamWithTime
                ...
                .window(TumblingEventTimeWindows.of(Time.seconds(Long.parseLong(interval)), Time.seconds(Long.parseLong(offset))))
    ...
    private static class CountAggregate
            implements AggregateFunction<Tuple2<String, Timestamp>, Long, Long> {
        ...
        @Override
        public Long add(Tuple2<String, Timestamp> value, Long accumulator) {
            // log.warn("accumulator" + accumulator.f0 + " " + accumulator.f1);
    ...
    private static class ProcessTumblingWindowFunction
            extends ProcessWindowFunction<Long, Tuple4<String, Long, Timestamp, Timestamp>, String, TimeWindow> {

        @Override
        public void process(String key,
                            Context context,
                            Iterable<Long> counts,
                            Collector<Tuple4<String, Long, Timestamp, Timestamp>> out) {
